...
void collect_streamed_ffn_hosts(const llama_model* model, uint32_t n_park,
    ...
    for (uint32_t layer = 0; layer < kNLayers; ++layer) {
        ...
        for (uint32_t i = 0; i < kStreamedBindSuffixCount; ++i) {
            ...
            // Slot A/B only packs the three Q4 GEMM weights.
...
bool attach_q4_to_ggml_slot(ggml_backend_buffer_t buf, ggml_tensor* t, const void* host,
    ...
    detach_cpu_mapped(t);
    t->buffer = buf;
    // ggml stores an alloc offset in t->data. A host mmap VA left here
    // is the post-reserve 0xC0000005. Start as offset, init, then
    // re-resolve to buffer base + off.
    t->data = reinterpret_cast<void*>(off);
    ggml_backend_buffer_init_tensor(buf, t);
    bool moved = false;
    void* resolved = repoint_cuda_data(base, cap, t->data, off, nbytes, &moved);
    if (resolved) {
        t->data = resolved;
    }
    t->view_src = nullptr;
...
FfnTripletBytes measure_ffn_triplet(const llama_model* model, uint32_t layer) {
    ...
    auto nbytes = [](ggml_tensor* t) -> uint64_t {
        return t ? ggml_nbytes(mmap_root(t)) : 0;
    };
    m.gate = nbytes(g);
    m.up = nbytes(u);
    m.down = nbytes(d);
...
// Bind this layer’s gate+up+down into one measured slot. No hop collapse.
uint32_t bind_layer_q4_triplet(const llama_model* model, uint32_t layer,
    ...
    Part parts[3] = {
        {"ffn_gate", find_ffn_weight(model, layer, "ffn_gate")},
        {"ffn_up", find_ffn_weight(model, layer, "ffn_up")},
        {"ffn_down", find_ffn_weight(model, layer, "ffn_down")},
    };
    if (!parts[2].t) {
        parts[2].t = find_ffn_weight(model, layer, "ffn_out");
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (nbytes == 0) {
            std::fprintf(stderr, "FFN_SLOT_BIND layer=%u stem=%s FAIL empty\n", layer,
                         parts[i].stem);
            continue;
        }
        ...
        if (!attach_q4_to_ggml_slot(buf, root, host.data(), nbytes, off)) {
            std::fprintf(stderr,
                         "FFN_SLOT_BIND layer=%u stem=%s FAIL n=%llu off=%llu cap=%llu\n",
                         layer, parts[i].stem, static_cast<unsigned long long>(nbytes),
        ...
        std::fprintf(stderr,
                     "FFN_SLOT_BIND layer=%u stem=%s name=%s buft=%s on_cuda=%d "
                     "cpu_mapped=%d\n",
    ...
    if (clocks && n_bound > 0) {
        clocks->set_real_h2d(ok);
    }
    return n_bound;
}

// Keep 62/63 gate+up+down bound through graph reserve. If buft is CUDA
// but data/view_src is still a host mmap or integer offset, re-resolve
// to buffer base + view_offs. One bind — no packed=0 second pass.
uint32_t keep_bound_cuda_weights(const llama_model* model, uint32_t layer, const char* when) {
    ...
    for (const char* stem : kStems) {
        ...
        const size_t nbytes = ggml_nbytes(t);
        TensorDataKind kind = classify_tensor_data_ptr(t->data, base, cap);
        const bool cuda = classify_backend_buft_name(buft) == BuftKind::Cuda;
        int fixed = 0;
        if (cuda && t->view_src &&
            classify_backend_buft_name(buft_name_of(t->view_src)) != BuftKind::Cuda) {
            t->view_src = nullptr;
            fixed = 1;
        }
        if (cuda && tensor_data_is_av_risk(kind) && base) {
            bool moved = false;
            void* p = repoint_cuda_data(base, cap, t->data, t->view_offs, nbytes, &moved);
            if (p && (moved || p != t->data)) {
                t->data = p;
                fixed = 1;
            }
            kind = classify_tensor_data_ptr(t->data, base, cap);
        }
        std::fprintf(stderr,
                     "FFN_DATA_KIND when=%s layer=%u name=%s buft=%s data=%s "
                     "view_src=%s view_buft=%s fixed=%d\n",
                     when, layer, t->name, buft, tensor_data_kind_name(kind), vs_name, vs_buft,
                     fixed);
        if (cuda && tensor_data_is_av_risk(kind)) {
            ++n_risk;
            std::fprintf(stderr, "%s\n",
                         format_ffn_av_split_line(static_cast<int32_t>(layer), t->name, buft,
                                                  vs_name, vs_buft, tensor_data_kind_name(kind),
                                                  fixed)
                             .c_str());
        }
        std::fprintf(stderr, "%s\n",
                     format_ffn_hop_line(static_cast<int32_t>(layer), t->name, buft, vs_name,
                                         vs_buft, classify_split_cause(t->name, buft, true))
                         .c_str());
    }
    return n_risk;
}
...
LiveForwardStatus LlamaCppLiveForwardBackend::run(TraceHooks& hooks, TraceStreamer& streamer,
    ...
    std::fprintf(stderr, "%s\n",
                 format_ffn_slot_bytes_line(kNLayers - 2, trip_b.gate, trip_b.up, trip_b.down,
                                            trip_b.slot)
                     .c_str());
    ledger = vram_ledger_measured_slots(trip_a.slot, trip_b.slot);
    std::fprintf(stderr, "%s\n", format_vram_ledger(ledger).c_str());
    if (gpu_buft) {
        ...
        std::fprintf(stderr,
                     ...
                     "(measure_then_alloc=1 extra_park=0 hop_collapse=0)\n",
    ...
    keep_bound_cuda_weights(model, kNLayers - 1, "after_bind");
    keep_bound_cuda_weights(model, kNLayers - 2, "after_bind");
    parked_cuda_n = 0;
    streamed_cpu_n = 0;
    streamed_cuda_n = 0;
    ...
    std::fprintf(stderr, "FFN_PLACE_AFTER_BIND parked_cuda=%u streamed_cpu=%u streamed_cuda=%u "
                         ...
                         "(measured A/B; no leftover collapse; extra_park=0)\n",
    ...
    if (!ctx) {
        ...
        print_split_why(sl, cpu_63, last_63_name, last_63_buft);
        print_layer_mul_mat_srcs(model, kNLayers - 1, &cpu_63, &last_63_name, &last_63_buft);
        keep_bound_cuda_weights(model, kNLayers - 1, "reserve_fail");
        keep_bound_cuda_weights(model, kNLayers - 2, "reserve_fail");
        llama_model_free(model);
        llama_backend_free();
        st.ok = false;
    ...
    keep_bound_cuda_weights(model, kNLayers - 1, "after_reserve");
    keep_bound_cuda_weights(model, kNLayers - 2, "after_reserve");
    if (!ggml_slots_live) {
    ...
    while (consumed < n_tok) {
        ...
        if (rc != 0) {
            keep_bound_cuda_weights(model, kNLayers - 1, "decode_fail");
            keep_bound_cuda_weights(model, kNLayers - 2, "decode_fail");
            print_split_why(sl, cpu_63, last_63_name, last_63_buft);
