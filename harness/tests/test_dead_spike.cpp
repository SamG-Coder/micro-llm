...
void test_dead_vs_spike_pack(TestContext& ctx) {
    ...
    // Pack 3 via hidden vectors (relative r >> 0.02).
    ...
    // Identity must not spike (relative r == 0).
    std::vector<float> id(8, 2.f);
    CHECK(ctx, hooks.on_delta_hidden(5, id.data(), id.data(), 8));
    CHECK(ctx, hooks.table().pack_is_dead(5));
    CHECK(ctx, hooks.table().pack(5).n_spike == 0);
